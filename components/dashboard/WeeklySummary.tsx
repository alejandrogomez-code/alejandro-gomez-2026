...
function Delta({ value, suffix = '' }: { value: number | null; suffix?: string }) {
  ...
  return (
    <span className="inline-flex items-center gap-1 text-xs text-mist-500 dark:text-mist-400">
      {positive ? (
      ...
      <CardContent className="space-y-5">
        <div>
          ...
          <div className="mt-2 flex flex-wrap items-center justify-between gap-2">
            <span className="tabular text-sm text-mist-500 dark:text-mist-400">
        ...
        <dl className="grid grid-cols-2 gap-4 border-t border-mist-100 pt-4 dark:border-mist-800 sm:grid-cols-3">
          <div>
            <dt className="text-sm text-mist-500 dark:text-mist-400">Pasos promedio</dt>
            <dd className="tabular mt-1 text-lg font-medium text-mist-900 dark:text-mist-100">
              {formatNumber(current.averageSteps)}
            </dd>
            <Delta value={previous ? current.averageSteps - previous.averageSteps : null} />
          </div>
          <div>
            <dt className="text-sm text-mist-500 dark:text-mist-400">Días con objetivo</dt>
            <dd className="tabular mt-1 text-lg font-medium text-mist-900 dark:text-mist-100">
              {current.stepGoalDays} / 7
            </dd>
          </div>
          <div>
            <dt className="text-sm text-mist-500 dark:text-mist-400">Registros de peso</dt>
            <dd className="tabular mt-1 text-lg font-medium text-mist-900 dark:text-mist-100">
        ...
        {habitRows.length > 0 ? (
          <ul className="space-y-2 border-t border-mist-100 pt-4 dark:border-mist-800">
            {habitRows.map((row) => (
